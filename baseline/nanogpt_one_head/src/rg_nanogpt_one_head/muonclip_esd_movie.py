...
"""Render a movie from WeightWatcher's native log-log ESD plot for one matrix.

For every selected checkpoint this command constructs a one-matrix model and
runs WeightWatcher with ``plot=True`` and ``savefig=savedir``. WeightWatcher
itself writes the log-log ESD as ``ww.layer{plot_id}.esd.png`` via
``save_fig(plt, "esd", plot_id, savedir)``. The movie uses exactly that file;
it does not infer a plot type, use ``get_ESD()``, or reconstruct the ESD.
"""
...
def _run_weightwatcher_esd_frame(
    row: pd.Series,
    *,
    matrix_name: str,
    output_root: Path,
    ...
    if "layer_id" not in details.columns:
        raise RuntimeError(
            "WeightWatcher details do not contain layer_id; cannot resolve "
            "the native ESD filename"
        )

    # For a one-matrix model the FrameworkLayer plot_id defaults to the
    # WeightWatcher layer_id. WeightWatcher's save_fig() writes exactly:
    #   {savedir}/ww.layer{plot_id}.esd.png
    plot_id = str(details.iloc[0]["layer_id"])
    if plot_id.endswith(".0"):
        plot_id = plot_id[:-2]
    esd_path = native_dir / f"ww.layer{plot_id}.esd.png"

    if not esd_path.is_file():
        generated = sorted(native_dir.glob("ww.layer*.esd.png"), key=_natural_key)
        raise FileNotFoundError(
            "WeightWatcher did not create the expected native log-log ESD file "
            f"{esd_path}. Generated ESD files: {[p.name for p in generated]}"
        )
...
def _make_smooth_frames(
    ...
    *,
    ...
    if any(image.shape != shape for image in images):
        raise RuntimeError(
            "native WeightWatcher ESD image dimensions changed across checkpoints"
        )
    ...
    for index in range(len(images) - 1):
        ...
        for subframe in range(frames_per_transition):
            ...
            eased = 0.5 - 0.5 * np.cos(np.pi * raw)
            blended = (1.0 - eased) * left + eased * right
            plt.imsave(
                movie_frames / f"frame_{frame_number:06d}.png",
                blended,
            )
            frame_number += 1
...
def _encode_mp4(frame_dir: Path, *, output_path: Path, fps: int) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Generate an MP4 from WeightWatcher's exact native log-log ESD "
            "figure for one MuonClip matrix."
        )
    )
    parser.add_argument(
        "--walk-dir",
    ...
    output_root = (
        ...
        / "diagnostics"
        / f"native_ww_esd_movie_{args.matrix}_{args.cadence}"
    )
    ...
    movie_frames = _make_smooth_frames(
        native_frames,
        output_root=output_root,
    ...
    print(f"[one-head-esd-movie] native ESD frames: {len(native_frames)}")
