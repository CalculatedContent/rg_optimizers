...
def _tpu_hardware_hint() -> bool:
    ...
    if pjrt.startswith("TPU"):
        return True
    # TPU_NAME is commonly exported on a laptop to manage a remote TPU. It is
    # evidence of local hardware only on Linux.
    ...
    return any(
        path.exists()
        for path in (Path("/dev/accel0"), Path("/sys/class/accel/accel0"))
    )
...
def _load_xla(*, required: bool) -> tuple[Any, Any, Any] | None:
    if importlib.util.find_spec("torch_xla") is None:
        if required:
            raise RuntimeError(
                "A TPU environment was requested or detected, but torch_xla "
                "is not installed. In the TPU VM run "
...
def is_tpu_environment(requested: str = "auto") -> bool:
...
def _choose_tpu_device() -> torch.device:
    ...
    observed = _xla_device_type(xr)
    if observed != "TPU":
        raise RuntimeError(
            "torch_xla initialized, but its PJRT device is not TPU. "
            f"Observed PJRT device: {observed or 'unknown'}"
        )
    device = torch_xla.device() if hasattr(torch_xla, "device") else "xla"
    return torch.device(device)


def choose_device(requested: str = "auto") -> torch.device:
    requested = str(requested).lower()
    requested = {"gpu": "cuda", "apple": "mps", "xla": "tpu"}.get(
        requested, requested
    )
    if requested == "auto":
    ...
    if requested == "cpu":
        return torch.device("cpu")
    raise ValueError(
        f"unsupported device request {requested!r}; choose auto, tpu, cuda, mps, or cpu"
    )
...
def configure_runtime(device: torch.device, cfg: dict) -> None:
    ...
    if device.type == "xla":
        ...
        if reduced:
            raise RuntimeError(
                "The reference protocol is float32, but TPU reduced precision "
                "is enabled through "
                + ", ".join(reduced)
                + ". Unset those variables; this protocol does not silently "
                "change numerical precision across accelerators."
            )
        modules = _load_xla(required=True)
        assert modules is not None
        torch_xla, xr, _ = modules
        torch_version = _major_minor(torch.__version__)
        xla_version = _major_minor(getattr(torch_xla, "__version__", ""))
        if (
        ...
        if count != 1:
            raise RuntimeError(
                ...
                f"Detected process_count={count}. Use a single-process launcher "
                "or implement an explicitly distributed protocol."
            )
...
def model_weight_norm(model: nn.Module) -> float:
    ...
    return 0.0 if total is None else math.sqrt(float(total.detach().cpu()))
...
def main() -> None:
    ...
    root_device = "tpu" if device.type == "xla" else device.type
    payload = {
        "runtime": runtime_metadata(device),
        "roots": {
            key: str(value)
            for key, value in roots(root_device).items()
        },
    }
