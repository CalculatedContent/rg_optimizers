...
def roots() -> dict[str, Path]:
    ...
    return {
        "root": root,
        "data": Path(os.environ.get("RG_NANOGPT_ONE_HEAD_DATA_ROOT", root / "data")).expanduser(),
        "results": Path(os.environ.get("RG_NANOGPT_ONE_HEAD_RESULTS_ROOT", root / "results")).expanduser(),
        "plots": Path(os.environ.get("RG_NANOGPT_ONE_HEAD_PLOTS_ROOT", root / "plots")).expanduser(),
    }
...
def validate_config(cfg: dict[str, Any]) -> None:
    ...
    if int(model["n_head"]) != 1:
        raise ValueError("this experiment is intentionally fixed to exactly one attention head")
    if int(model["n_layer"]) != 1:
        raise ValueError("this experiment is intentionally fixed to one transformer block")
    ...
    for key in ("train_tokens", "val_tokens", "test_tokens"):
        if int(dataset[key]) <= int(model["block_size"]) + 1:
            raise ValueError(f"dataset.{key} is too small for the configured context length")
    ...
    for key in ("bleu_examples", "bleu_prompt_tokens", "bleu_continuation_tokens", "bleu_batch_size"):
    ...
    if int(evaluation["bleu_prompt_tokens"]) + int(evaluation["bleu_continuation_tokens"]) > int(model["block_size"]):
        raise ValueError("BLEU prompt plus continuation must fit inside model.block_size")

    ww = cfg["weightwatcher"]
    if not bool(ww.get("ERG", False)):
        raise ValueError("WeightWatcher ERG must be enabled for this reference experiment")
    if not bool(ww.get("randomize", False)):
        raise ValueError("WeightWatcher randomize must be enabled to obtain num_traps")
...
def optimizer_profile(cfg: dict[str, Any], optimizer: str) -> dict[str, Any]:
    ...
    if optimizer not in SUPPORTED_OPTIMIZERS:
        raise ValueError(f"unsupported optimizer {optimizer!r}; choose from {SUPPORTED_OPTIMIZERS}")
