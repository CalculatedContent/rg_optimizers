...
from .training import run_directory, run_is_complete
...
# Matrix identity remains fixed across every optimizer notebook.
MATRIX_COLORS = {
    "W_Q": "#0072B2",
    "W_K": "#E69F00",
    "W_V": "#009E73",
            ...
            rows.append(
                {
                    ...
                    "final_test_bleu": payload.get("final_test_bleu", np.nan),
                    "run_dir": str(run_dir),
                }
            )
...
def load_epoch_metrics(
    ...
    *,
    ...
    if frame.empty:
        return frame
    return frame.sort_values(["optimizer", "seed", "nominal_epoch"]).drop_duplicates(
        ["optimizer", "seed", "nominal_epoch"], keep="last"
    )
...
def load_layer_metrics(
    ...
    *,
    ...
    if frame.empty:
        return frame
    return frame.sort_values(["optimizer", "seed", "epoch", "matrix_type"]).drop_duplicates(
...
def plot_epoch_metric(
    ...
    *,
    ...
    axis.set_title(title or f"{metric}: mean and 95% Student-t CI across seeds")
...
def plot_spectral_optimizer_summary(
    ...
    *,
    ...
    return plot_epoch_metric(
        ...
        x="epoch",
        optimizers=optimizers,
        title=f"WeightWatcher {metric}: mean and 95% Student-t CI",
        output=output,
    )
