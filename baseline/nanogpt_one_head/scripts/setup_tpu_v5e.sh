...
if ((FORCE_FRAMEWORK_REINSTALL)) || ! framework_matches; then
...
ENV_DIR="${HOME}/.config/rg_optimizers"
ENV_FILE="${ENV_DIR}/tpu_env.sh"
mkdir -p "$ENV_DIR"
{
  printf '# Generated by %s. Safe to source from ~/.bashrc.\n' "$SCRIPT_NAME"
...
BASHRC_LINE='[ -f "$HOME/.config/rg_optimizers/tpu_env.sh" ] && source "$HOME/.config/rg_optimizers/tpu_env.sh"'
...
[[ -n "$RG_ENV" ]] || RG_ENV="${HOME}/.local/bin/rg-onehead-env"
...
PY
  ...
  [[ -n "$RG_PREPARE" ]] || RG_PREPARE="${HOME}/.local/bin/rg-onehead-prepare"
  [[ -n "$RG_TRAIN" ]] || RG_TRAIN="${HOME}/.local/bin/rg-onehead-train"
