...
class BaselineConfig:
    ...
    The defaults are deliberately optimizer-specific. Each profile uses a short
    linear warm-up followed by cosine decay to a non-zero floor. The
    ``sgd_momentum_muon`` key is retained for backwards-compatible result paths;
    the implementation is Muon on the two hidden matrices with auxiliary
    AdamW, matching the reference Muon parameter partition.
    """
    ...
    # Correlation traps come from randomized MP analysis in randomize=True.
    ww_min_evals: int = 8
    ...
    def validate(self) -> None:
        ...
        if self.epochs < 2:
            raise ValueError("epochs must be at least two for warm-up/cosine schedules")
        ...
        if self.schedule != "warmup_cosine":
            raise ValueError("only the preregistered warmup_cosine schedule is supported")
        ...
        for name, value in {
            ...
            if value < 0 or value >= self.epochs:
                raise ValueError(f"{name} must satisfy 0 <= warmup < epochs")
    ...
    def run_slug(self) -> str:
        # Keep the historical directory key so existing comparison code and
        # prior result stores remain discoverable.
