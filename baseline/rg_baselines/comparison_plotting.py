"""Color-consistent plots for persisted baseline optimizer comparisons."""
...
PERFORMANCE_PLOTS: tuple[tuple[str, str, str, float | None, bool], ...] = (
    ...
    ("train_accuracy", "Train accuracy (%)", "02_train_accuracy_95ci.png", None, True),
    ("test_loss", "Test cross-entropy", "03_test_loss_95ci.png", None, False),
    ("train_loss", "Train cross-entropy", "04_train_loss_95ci.png", None, False),
    (
        "test_perplexity",
        "Test classification perplexity, exp(CE)",
        "05_test_perplexity_95ci.png",
        None,
        False,
    ),
    (
        ...
        "06_accuracy_generalization_gap_95ci.png",
        0.0,
        True,
    ),
    (
        ...
        "07_loss_generalization_gap_95ci.png",
        0.0,
        False,
    ),
)
...
SPECTRAL_PLOTS: tuple[tuple[str, str, str, float | None], ...] = (
    ("alpha", "WeightWatcher alpha", "08_weightwatcher_alpha_95ci.png", 2.0),
    (
        "detX_num",
        "WeightWatcher detX retained count",
        "09_weightwatcher_detx_num_95ci.png",
        None,
    ),
    (
        "num_pl_spikes",
        "WeightWatcher power-law spike count",
        "10_weightwatcher_num_pl_spikes_95ci.png",
        None,
    ),
    ("ERG_gap", "WeightWatcher ERG gap", "11_erg_gap_95ci.png", 0.0),
    ("m_midpoint", "Midpoint retained rank", "12_midpoint_rank_95ci.png", None),
    (
        "trace_log_midpoint_per_eval",
        "Midpoint trace-log per retained eigenvalue",
        "13_trace_log_midpoint_per_eval_95ci.png",
        0.0,
    ),
    ("stable_rank", "Stable rank", "14_stable_rank_95ci.png", None),
)
...
def plot_performance_metric(
    ...
    *,
    ...
    for optimizer in optimizer_order:
        ...
        for seed in seeds:
            rows = raw.loc[raw["seed"].astype(int).eq(int(seed))].sort_values("epoch")
            values = rows[metric].astype(float).to_numpy()
            if percent:
                values = 100.0 * values
            ax.plot(rows["epoch"], values, color=color, linewidth=0.9, alpha=0.18)

        rows = summary.loc[
            summary["optimizer"].eq(optimizer) & summary["metric"].eq(metric)
        ].sort_values("epoch")
        ...
        if percent:
            mean, low, high, half = [100.0 * values for values in (mean, low, high, half)]
    ...
    if horizontal_reference is not None:
        ...
        ax.axhline(value, color="black", linewidth=1.0, linestyle="--", alpha=0.6)
    ax.set(xlabel="Epoch", ylabel=ylabel, title=f"{ylabel} across optimizer baselines")
    ax.grid(alpha=0.2)
...
def plot_spectral_metric(
    ...
    *,
    ...
    fig, axes = plt.subplots(1, len(layer_order), figsize=(16.0, 4.8), sharex=True)
    for ax, layer in zip(axes, layer_order):
        for optimizer in optimizer_order:
            ...
            for seed in seeds:
                rows = raw.loc[raw["seed"].astype(int).eq(int(seed))].sort_values("epoch")
                ax.plot(rows["epoch"], rows[metric], color=color, linewidth=0.8, alpha=0.15)
