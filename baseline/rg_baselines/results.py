...
def validate_result(result: BaselineResult) -> None:
    ...
    if set(result.performance.epoch.astype(int)) != epochs:
        raise RuntimeError("performance epochs incomplete")
    performance = {"train_loss", "train_accuracy", "test_loss", "test_accuracy"}
    if performance - set(result.performance):
        raise RuntimeError("required train/test metrics missing")
    if result.performance[list(performance)].isna().any().any():
        raise RuntimeError("train/test metric contains NaN")
    ...
    if spectral - set(result.spectral_metrics):
        raise RuntimeError("required spectral metrics missing")
    ...
    for epoch in epochs:
        ...
        if not {"fc1", "fc2", "fc3"}.issubset(layers):
            raise RuntimeError(f"epoch {epoch} missing layers")
    ...
    expected_gap = valid.detX_num.astype(int) - valid.num_pl_spikes.astype(int)
    if not np.array_equal(expected_gap.to_numpy(), valid.ERG_gap.astype(int).to_numpy()):
        raise RuntimeError("ERG_gap audit failed")
    midpoint = np.floor(
        (valid.detX_num.astype(float) + valid.num_pl_spikes.astype(float)) / 2
    ).astype(int)
