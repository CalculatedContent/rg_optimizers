...
from rg_nanogpt_one_head.checkpoints import load_training_checkpoint, save_training_checkpoint
from rg_nanogpt_one_head.evaluation import fixed_bleu_probe, fixed_probe
from rg_nanogpt_one_head.optimizers import make_optimizer_handles
from rg_nanogpt_one_head.runtime import choose_device, configure_runtime, seed_everything
...
def run_one(
    *,
    ...
    if run_dir.exists() and not resume:
        raise FileExistsError(f"run directory exists: {run_dir}; enable resume or overwrite")
    ...
    matrix_count = expected_matrix_count(cfg)
    ...
    if completion_path.is_file():
        ...
        if progress:
            print(f"[ngb-train] reuse verified completed {cfg['protocol']['name']} {optimizer_name} seed={seed}")
        return run_dir
    ...
    train_probe = fixed_probe(arrays["train"], batch_size=batch_size, block_size=block_size, n_batches=eval_batches, seed=int(eval_cfg["train_probe_seed"]))
    val_probe = fixed_probe(arrays["val"], batch_size=batch_size, block_size=block_size, n_batches=eval_batches, seed=int(eval_cfg["validation_probe_seed"]))
    test_probe = fixed_probe(arrays["test"], batch_size=batch_size, block_size=block_size, n_batches=eval_batches, seed=int(eval_cfg["test_probe_seed"]))
    ...
    if resume and latest_checkpoint.is_file():
        ...
        if progress:
            print(f"[ngb-train] resume {cfg['protocol']['name']} {optimizer_name} seed={seed} step={start_step}")
    ...
    (run_dir / "test_results.json").write_text(json.dumps(test_results, indent=2, sort_keys=True), encoding="utf-8")
    completion = {
        "completed": True,
    ...
    temporary.write_text(json.dumps(completion, indent=2, sort_keys=True), encoding="utf-8")
    temporary.replace(completion_path)
    if progress:
        print(f"[ngb-train] complete {cfg['protocol']['name']} {optimizer_name} seed={seed}: {run_dir}")
    return run_dir
