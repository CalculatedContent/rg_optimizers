...
from rg_nanogpt_one_head.checkpoints import save_epoch_model_checkpoint, save_training_checkpoint
from rg_nanogpt_one_head.evaluation import evaluate_bleu, evaluate_probe, random_batch
from rg_nanogpt_one_head.optimizers import optimizer_step, set_learning_rates, zero_grad
...
def execute_training_loop(
    *,
    ...
    last_update_lrs = {
        "primary": 0.0,
        "auxiliary": 0.0 if any(handle.role == "auxiliary" for handle in handles) else float("nan"),
    }
    if start_step > 0:
    ...
    for completed_steps in range(start_step, total_steps + 1):
        ...
        if evaluation_due:
            ...
            test_metrics = {"loss": float("nan"), "perplexity": float("nan"), "accuracy": float("nan")}
            ...
            if epoch_due:
                ...
                epoch_writer.writerow({**row, "nominal_epoch": nominal_epoch, "checkpoint_path": str(checkpoint_path), "test_monitoring_only": 1})
                epoch_handle.flush()
                ...
                if progress:
                    print(
                        "[ngb-ww] "
                        f"protocol={cfg['protocol']['name']} optimizer={optimizer_name} seed={seed} "
                        f"epoch={nominal_epoch:.2f} alpha={ww_summary.get('alpha_median', float('nan')):.3f} "
            ...
            if progress:
                ...
                print(
                    "[ngb-train] "
                    f"protocol={cfg['protocol']['name']} optimizer={optimizer_name} seed={seed} "
                    f"step={completed_steps}/{total_steps} epoch={actual_epoch:.3f} "
                    f"last_lr={last_update_lrs.get('primary', float('nan')):.3e} "
                    f"next_lr={next_update_lrs.get('primary', float('nan')):.3e} "
                    f"train_loss={train_metrics['loss']:.4f} val_loss={val_metrics['loss']:.4f} "
                    f"val_ppl={val_metrics['perplexity']:.2f} val_acc={100 * val_metrics['accuracy']:.2f}% eta={eta_text}",
                    flush=True,
                )
    ...
    return float(best_validation_loss), int(best_validation_step), float(elapsed_offset + time.time() - started)
