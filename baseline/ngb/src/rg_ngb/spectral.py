...
class WeightMatrixHolder(nn.Module):
    def __init__(self, model: GPT) -> None:
        ...
        for name, matrix_type, block, weight in transformer_matrix_items(model):
            ...
            self.matrix_metadata.append({"matrix_name": name, "matrix_type": matrix_type, "block": int(block)})


def _attach_matrix_metadata(frame: pd.DataFrame, metadata: list[dict[str, object]]) -> pd.DataFrame:
    ...
    if any(value is None for value in resolved) or len(result) != len(metadata):
        raise RuntimeError(
            f"WeightWatcher returned {len(result)} rows for {len(metadata)} expected transformer matrices"
        )
...
def _restore_accelerator_rng(state: dict[str, Any]) -> None:
    ...
    if "mps" in state and hasattr(torch, "mps") and hasattr(torch.mps, "set_rng_state") and torch.backends.mps.is_available():
        torch.mps.set_rng_state(state["mps"])


def summarize_spectral_frame(frame: pd.DataFrame, *, step: int, tokens_seen: int, epoch: float) -> dict[str, Any]:
    ...
    for metric in SPECTRAL_METRICS:
        values = pd.to_numeric(frame[metric], errors="coerce") if metric in frame.columns else pd.Series(dtype=float)
...
def run_weightwatcher(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("WeightWatcher is required; install NGB dependencies in the active conda environment") from exc

    run_dir = Path(run_dir)
    spectral_root = run_dir / "spectral"
    raw_root = spectral_root / "raw"
    raw_root.mkdir(parents=True, exist_ok=True)
    raw_path = raw_root / f"weightwatcher_step_{int(step):07d}.csv"
    if raw_path.is_file():
        frame = pd.read_csv(raw_path)
        return summarize_spectral_frame(frame, step=step, tokens_seen=tokens_seen, epoch=tokens_seen / max(1, int(train_tokens)))
    ...
    try:
        ...
        if missing:
            raise RuntimeError("WeightWatcher omitted required columns: " + ", ".join(missing))
        required_values = frame[list(required)].apply(pd.to_numeric, errors="coerce")
        if not np.isfinite(required_values.to_numpy(dtype=float)).all():
            raise RuntimeError("WeightWatcher required alpha/ERG_gap/num_traps values are non-finite")
        ...
        frame.insert(3, "diagnostic_seed", int(diagnostic_seed))
        _atomic_csv(raw_path, frame)
        _append_deduplicated(spectral_root / "layers.csv", frame, keys=["step", "matrix_name"])
        summary = summarize_spectral_frame(frame, step=step, tokens_seen=tokens_seen, epoch=epoch)
        _append_deduplicated(spectral_root / "summary.csv", pd.DataFrame([summary]), keys=["step"])
        status = {
            "step": int(step),
            "tokens_seen": int(tokens_seen),
            "epoch": float(epoch),
            "completed": True,
            "matrix_count": len(holder.matrix_metadata),
            "raw_path": str(raw_path),
        }
        (spectral_root / f"status_step_{int(step):07d}.json").write_text(json.dumps(status, indent=2, sort_keys=True), encoding="utf-8")
        return summary
    except Exception as exc:
        status = {"step": int(step), "tokens_seen": int(tokens_seen), "completed": False, "error_type": type(exc).__name__, "error": str(exc)}
        (spectral_root / f"status_step_{int(step):07d}.json").write_text(json.dumps(status, indent=2, sort_keys=True), encoding="utf-8")
        if bool(config.get("strict", True)):
            raise
        print(f"[ngb-ww] WARNING step={step}: {type(exc).__name__}: {exc}", flush=True)
        return status
    finally:
