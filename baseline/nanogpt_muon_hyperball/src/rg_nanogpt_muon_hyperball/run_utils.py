...
from .config import lr_schedule_steps, optimizer_profile, tokens_per_step
...
METRIC_FIELDS = [
    ...
    "hyperball_active_fraction", "hyperball_mean_scale",
    "hyperball_min_scale", "hyperball_mean_radius",
...
def prepare_csv(path: Path, fields: list[str], resume_step: int | None) -> None:
    ...
    if path.is_file() and resume_step is not None:
        ...
        if "step" in frame.columns:
            frame = frame[
                pd.to_numeric(frame["step"], errors="coerce") < int(resume_step)
            ]
...
def truncate_spectral_after(run_dir: Path, resume_step: int) -> None:
    ...
    for filename in ("layers.csv", "summary.csv"):
        ...
        if path.is_file():
            ...
            if "step" in frame.columns:
                frame = frame[
                    pd.to_numeric(frame["step"], errors="coerce")
                    < int(resume_step)
                ]
            temporary = path.with_suffix(path.suffix + ".tmp")
            frame.to_csv(temporary, index=False)
    ...
    *,
    ...
    payload = {
        ...
        "lr_schedule_steps": int(
            lr_schedule_steps(cfg, optimizer_profile(cfg, optimizer_name))
        ),
        ...
        "hyperball_policy": (
            "Muon proposes the complete hidden-matrix displacement; a relative "
