"""New ten-run protocol. Original trainers and checkpoints are not modified."""
...
from am_spectral import measure,offenders,save_rows


def snapshot(model,handles,counts,step,scales,attempted):
    return {'model':cpu_copy(model.state_dict()),'optimizers':[cpu_copy(h.optimizer.state_dict()) for h in handles],
            'counts':dict(counts),'step':step,'scales':dict(scales),'attempted':attempted,
...
def split_groups(model,handles):
    names={id(p):n for n,p in model.named_parameters()}
    for h in handles:
        groups=[]
        for g in h.optimizer.param_groups:
            for p in g['params']:
                group={k:v for k,v in g.items() if k!='params'}
                group.update(params=[p],name=names[id(p)]); groups.append(group)
        h.optimizer.param_groups=groups
...
def _train(cfg,run_dir,arm,seed,device,resume):
    ...
    optimizer='adamw' if arm=='adamw' else 'muon'
    profile=copy.deepcopy(source['optimizer_profiles'][optimizer])
    handles=make_handles(model,profile); split_groups(model,handles)
    manifest={'protocol':cfg,'source_model':asdict(model.cfg),'profile':profile,'arm':arm,'optimizer':optimizer,
    ...
    peer=run_dir.parent.parent/('muon_guarded' if arm=='adamw' else 'adamw')/run_dir.name/'manifest.json'
    if peer.exists():
    ...
    counts=Counter(); scales={n:1. for n,p in model.named_parameters()}; first=0; attempted=0
    if resume and latest.exists():
        ...
        restore(saved,model,handles); first=saved['step']; counts=Counter(saved['counts'])
        scales=saved['scales']; attempted=saved['attempted']
    ...
    started=time.monotonic(); completed=first
    try:
        initial=measure(model,cfg['weightwatcher'],seed,first)
        if arm=='muon_guarded' and offenders(initial,cfg['guard']['target']):
            save_rows(run_dir/'rejected_spectra'/f'initial_{first:08d}.csv',initial)
            raise RuntimeError('Initial/restart alpha gate failed; seed is retained, never resampled.')
        save_rows(run_dir/'spectral'/f'step_{first:08d}.csv',initial)
        if not latest.exists():
            saved=snapshot(model,handles,counts,0,scales,attempted)
            saved.update(fingerprint=manifest['fingerprint'],model_sha256=model_hash(model))
            atomic_save(latest,saved)
        if first%cfg['behavior_every']==0 or first==cfg['steps']:
            audit(model,data,cfg,source,run_dir,first,counts)
        while completed<cfg['steps']:
            stop=min(cfg['steps'],((completed//cfg['spectral_every'])+1)*cfg['spectral_every'],
                     data.withdrawal if completed<data.withdrawal else cfg['steps'])
            saved=snapshot(model,handles,counts,completed,scales,attempted)
            for retry in range(cfg['guard']['max_retries']+1):
                restore(saved,model,handles); counts=Counter(saved['counts']); model.train()
                last_loss=None
                for step in range(completed,stop):
                    attempted+=1; zero_grad(handles)
                    lrs=set_lrs(handles,update_index=step,total_steps=schedule_steps,warmup_steps=warmup)
                    for h in handles:
                        for group in h.optimizer.param_groups: group['lr']*=scales[group['name']]
                    ...
                    try:
                        ...
                        state=snapshot(model,handles,counts,step,scales,attempted)
                        ...
                        atomic_save(run_dir/'failure_state.pt',state)
                        raise
                    counts.update(r.id for r in records)
                    if (step+1)%25==0:
                        print(f'{arm} seed={seed} step={step+1}/{cfg["steps"]} loss={last_loss:.6f} '
                              f'grad_norm={norm:.3g} norm_fallback={overflow} retry={retry}',flush=True)
                rows=measure(model,cfg['weightwatcher'],seed,stop)
                bad=offenders(rows,cfg['guard']['target']) if arm=='muon_guarded' else []
                event={'start':completed,'stop':stop,'retry':retry,'accepted':not bad,
                       'offending_matrices':bad,'attempted_updates':attempted,'scales':dict(scales)}
                with (run_dir/'control_events.jsonl').open('a') as f: f.write(json.dumps(event)+'\n')
                if not bad: break
                save_rows(run_dir/'rejected_spectra'/f'{stop:08d}_{attempted:09d}.csv',rows)
                for name in bad: scales[name]*=cfg['guard']['backoff']
                print(f'ROLLBACK {arm} step={stop} matrices={bad}',flush=True)
            else:
                restore(saved,model,handles)
                raise RuntimeError('Guard retry limit reached; no below-threshold window was accepted.')
            for r in data.canaries:
                if counts[r.id]!=data.planned_counts(stop)[r.id]: raise RuntimeError('Realized dose disagrees with schedule.')
            completed=stop
            save_rows(run_dir/'spectral'/f'step_{completed:08d}.csv',rows)
            state=snapshot(model,handles,counts,completed,scales,attempted)
            state.update(fingerprint=manifest['fingerprint'],model_sha256=model_hash(model))
            atomic_save(latest,state)
            if completed%cfg['expensive_every']==0 or completed in (data.withdrawal,cfg['steps']):
            ...
            print(f'ACCEPT {arm} seed={seed} step={completed} WW=clip_xmax matrices={len(rows)}',flush=True)
        ...
        atomic_json(run_dir/'complete.json',result)
        return result
    except (Exception,KeyboardInterrupt) as exc:
        result={'state':'interrupted' if isinstance(exc,KeyboardInterrupt) else 'failed',
                'last_accepted_step':completed,'error':str(exc),'attempted_updates':attempted}
