...
class WrapperTests(unittest.TestCase):
    def _run_once(self, base_factory):
        ...
        self.assertTrue(all(row.get("status") == "ok" for row in stats if "parameter" in row))
        after = {name: p.detach().clone() for name, p in model.named_parameters() if p.ndim == 2}
        self.assertTrue(any(not torch.allclose(before[name], after[name]) for name in before))
        self.assertTrue(any("orthogonal_fraction" in row for row in stats))

    def test_adamw_wrapper(self):
        self._run_once(lambda model: torch.optim.AdamW(model.parameters(), lr=1e-3))

    def test_sgd_momentum_wrapper(self):
        self._run_once(lambda model: torch.optim.SGD(model.parameters(), lr=1e-2, momentum=0.9))
